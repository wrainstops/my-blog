    ...
    <div class="main-container">
      <div class="waves">
        <div class="wave" v-for="(item, key) in waves" :key="key" :style="{ ...item }">
          <div v-for="n in wavesConfig.total" :key="n" class="wave-item" :style="{
...
export default {
  ...
  mounted() {
    ...
    let lastCount = 0;// 2秒内无点击清空waves，防止过多的dom累积占用内存
    setInterval(() => {
  ...
  methods: {
    createWave(e) {
      ...
      // 在一定范围内随机生成波纹的大小
      const waveSize = parseInt(Math.random() * (this.wavesConfig.maxSize - this.wavesConfig.minSize) + this.wavesConfig.minSize);
      //添加新的波纹数据
      this.waves.push({
